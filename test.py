...
class TestBlockStructure(unittest.TestCase):        
    ...
    def run_test(self, in_filepath, expected_filepath):
        ...
        for i in range(len(expected)):
            if expected[i][0].startswith("$"):
                s_expected = sorted(re.findall("\(-?\d+, -?\d+\)", expected[i][0][1:]))
                s_output = sorted(re.findall("\(-?\d+, -?\d+\)", stdin_output[i]))
                

                if s_expected != s_output:
                    raise AssertionError(" ".join(s_expected) + " not equal to " + " ".join(s_output))
                
                
            elif stdin_output[i] not in expected[i]:
